...
| MACE hybrid research | `mace_polar_1m_vacuum_r2scan3c_subtractive_pilot_v1`; `scripts/mace_hybrid.py` | All 12 calls completed on A5000 (1200381). Full 9,141-atom endpoints: ~58 s each, 9.55 GiB GPU, <=1.65 GiB host RSS. Memory blocking preserves original core energies/forces/densities; 17 tests pass. Repeat/translation/charge pass; rotation and hybrid partition (-5.00 kcal/mol versus 2 target) fail frozen gates. No calibrated class or accuracy gain claimed. Rotation audit 1200396 identifies fixed-axis displaced charges as an error source; original/blocked rotated cores match within ~1e-11. Analytic medium replacement completed as 1200470: all core/full rotation and charge checks pass; ~58 s/full endpoint, 9.55 GiB GPU. Partition residual -5.023 kcal/mol still fails 2 target. Baseline unchanged. Large-checkpoint follow-up in preparation under standing pilot approval. [Analytic results](../diagnostics/mace_analytic_20260916/REPORT.md). [Rotation audit](../diagnostics/mace_rotation_20260916/REPORT.md). [Results](../diagnostics/mace_hybrid_20260916/MEMORY_RESULTS.md). [Runbook](../diagnostics/mace_hybrid_20260916/RUNBOOK.md). |
