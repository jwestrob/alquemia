...
def choose_target(w,target,fold,root,sources):
    ...
    failure_status='preparation_failed';failure_stage='folding_output'
    review.update(upstream_fold_state=fold.get('state') if fold else None,upstream_fold_reason=fold.get('reason') if fold else 'Missing target record')
    try:
        ...
        if failure_status=='preparation_failed' and review['upstream_fold_reason']:
            reason+='; upstream: '+str(review['upstream_fold_reason'])
        review.update(status=failure_status,failure_stage=failure_stage,reason=reason)
        write(directory/'selection.json',review)
        return None,{'target_id':uid,'status':failure_status,'failure_stage':failure_stage,'reason':reason,
           'upstream_fold_state':review['upstream_fold_state'],'selection_review':record(directory/'selection.json')}
...
def prepare_worker(payload):
    ...
    try:
        ...
        reason=f'{type(exc).__name__}: {exc}'
        result={'case':None,'outcome':{'target_id':target['target_id'],'status':'preparation_failed','failure_stage':'chemical_preparation','reason':reason,'case_id':cid},
...
def dispatch_worker(payload):
    ...
    try:
        ...
        result={'case':None,'outcome':{'target_id':uid,'status':'preparation_failed','failure_stage':'preparation_process','reason':failure,'case_id':target['case_id']},
...
def main(argv=None):
    ...
    counts=dict(collections.Counter(x['status'] for x in outcomes))
    unsupported=[x for x in outcomes if x['status']=='unsupported']
    failed=[x for x in outcomes if x['status']=='preparation_failed']
    validation=root/'geometry_validation.json';write(validation,{'status':'PASS','scope':'All admitted cases pass geometry; failed and unsupported targets remain unscored',
        'ready_count':len(cases),'unsupported_count':len(unsupported),'failed_count':len(failed),'cases':{r['outcome']['case_id']:r['geometry'] for r in results if r['case']},
    ...
    write(root/'prepared_pairs.json',{'schema_version':'plm.adh9.prepared_pairs.v1','protocol_id':PROTOCOL,
       ...
       'reused_results':reused,'reused_count':len(reused),'unsupported_targets':unsupported,'unsupported_count':len(unsupported),
       'failed_targets':failed,'failed_count':len(failed),'orca_executed':False})
