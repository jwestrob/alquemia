...
def dry_run(manifest):
    if read_json(manifest).get("schema_version") == "alquemia.mace_metal_minimum_short.v1":
        from mace_metal_minimum import validate
        return validate(manifest)
    if read_json(manifest).get('schema_version', '').startswith('alquemia.mace_metal_response_'):
        from mace_metal_response import validate
        return validate(manifest)
...
def accepted_attempt(attempt, task, manifest):
    try:
        ...
        omol_components=('MACE_OMOL_total_vacuum_energy','MACE_OMOL_charge_feature_ablated_descriptor',
                         'MACE_OMOL_shared_neutral_charge_feature_descriptor')
    ...
    with (root/'execute.lock').open('a') as lock:
        ...
        for t in m['tasks']:
            ...
            if t['kind'] == 'full' and m['model'].get('pair_kernel'):
                if m.get('schema_version') in ('alquemia.mace_global_benchmark.v1', 'alquemia.mace_short_engine.v1', 'alquemia.mace_mechanics_short.v1', 'alquemia.mace_mechanics_minimum_short.v1', 'alquemia.mace_density_short.v1', 'alquemia.mace_metal_response_short.v1', 'alquemia.mace_metal_minimum_short.v1'):
