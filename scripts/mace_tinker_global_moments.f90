...
program alquemia_tinker_global_moments
  ...
  if (command_argument_count() /= 3) error stop 'expected XYZ MASK moments'
  call get_command_argument(2, mask_path)
  call get_command_argument(3, action)
  if (trim(action)/='moments') error stop 'unknown mode'
  call initial
  call getxyz
  call mechanic
  ...
  call final
end program alquemia_tinker_global_moments
