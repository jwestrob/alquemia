! Native direct-field accounting control. No native library/source modification.
! fields: direct fields only; static: one constrained no-response energy call.
program alquemia_tinker_field_accounting
  ...
  real*8, allocatable :: field(:,:),fieldp(:,:),fields(:,:),fieldps(:,:)
  character(len=6) :: switch_mode

  if (command_argument_count() /= 3) error stop 'expected XYZ MASK fields|static'
  call get_command_argument(2, mask_path)
  call get_command_argument(3, action)
  if (trim(action)/='fields' .and. trim(action)/='static') error stop 'unknown mode'
  call initial
  call getxyz
  call mechanic
  ...
  if (use_list.or.use_exfld) error stop 'unexpected list/external field'
  ...
  if (trim(action)=='fields') then
    ...
    allocate(field(3,n),fieldp(3,n),fields(3,n),fieldps(3,n))
    call dfield0d(field,fieldp,fields,fieldps)
    call cpu_time(end_cpu)
    call system_clock(end_clock)
    do i=1,n
      write(*,*) 'FIELD',i,rborn(i),field(:,i),fieldp(:,i),fields(:,i),fieldps(:,i)
    end do
  else
    ! energy()->esolv() reads uinds even without polarization; arrays are zero above.
    use_polar=.false.
    douind=.false.
    write(*,*) 'ALQUEMIA_STATIC_BEGIN'
    flush(6)
    call system_clock(start_clock,clock_rate)
    call cpu_time(begin_cpu)
    total_energy=energy()
  ...
  write(*,*) 'FINAL_STATE',use_polar,count(douind)
  write(*,*) 'KERNEL_TIMING',real(end_clock-start_clock,8)/real(clock_rate,8),end_cpu-begin_cpu
  write(*,*) 'ALQUEMIA_ACCOUNTING_COMPLETE ',trim(action),n,frozen_count
  call final
end program alquemia_tinker_field_accounting
