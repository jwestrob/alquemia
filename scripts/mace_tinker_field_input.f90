! Native induced-response input adapter; no energy() or force call.
program alquemia_tinker_field_input
  ...
  real*8, allocatable :: supplied(:,:,:),expected_born(:)
  character(len=2048) :: mask_path,field_path
  logical, allocatable :: frozen(:)

  if (command_argument_count() /= 3) error stop 'expected XYZ MASK FIELDS'
  call get_command_argument(2, mask_path)
  call get_command_argument(3, field_path)
  call initial
  call getxyz
  call mechanic
  ...
  if (ios/=0) error stop 'missing direct field file'
  read(mask_unit,*,iostat=ios) index
  if (ios/=0.or.index/=n) error stop 'field atom count mismatch'
  do i=1,n
    read(mask_unit,*,iostat=ios) index,expected_born(i),supplied(:,i,1), &
      supplied(:,i,2),supplied(:,i,3),supplied(:,i,4)
    if (ios/=0.or.index/=i) error stop 'field atom order/read mismatch'
    if (.not.all(ieee_is_finite(supplied(:,i,:)))) error stop 'nonfinite fields'
  end do
  read(mask_unit,*,iostat=ios) index
  if (ios>=0) error stop 'extra/malformed field data'
  close(mask_unit)
  call replica(0d0)
  call born
  call chkpole
  call rotpole('MPOLE')
  if (.not.all(ieee_is_finite(expected_born))) error stop 'nonfinite input Born radius'
  if (any(abs(rborn-expected_born)>1d-12)) error stop 'field cavity mismatch'
  debug=.true.
  ...
  call alquemia_induce0c(supplied)
  ...
  write(*,*) 'ALQUEMIA_RESPONSE_COMPLETE',n,frozen_count
  call final
end program alquemia_tinker_field_input
