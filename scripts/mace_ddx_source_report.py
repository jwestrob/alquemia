...
def report(collection,output):
    ...
    for g in m['groups']:
        ...
        for name,expected in expected_parameters.items():
            ...
            if recovery:continue # audited per state below, including original reused iteration settings
        ...
        for label in g['states']:
            ...
            energy=.5*math.fsum((psi*x).ravel());expected=np.zeros_like(psi);expected[0]=np.sqrt(4*np.pi)*q
        ...
        if all(result['rows'].get(x,{}).get('status')=='computed' for x in ('Ca','La')):
            a=arrays[gid];err=.5*abs(math.fsum((a['Ca_psi']*a['La_x']).ravel())-math.fsum((a['La_psi']*a['Ca_x']).ravel()))*HA_TO_KCAL
    ...
    for case in CASES:
        ...
        native_differences[case]=dict(native_GK=native,PCM_primary_minus_GK={k:primary[k]-native[k] if primary[k] is not None else None for k in native})
    left,right=CASES
    for grid in GRIDS:
