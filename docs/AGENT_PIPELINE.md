...
| MACE hybrid research | `mace_polar_1m_vacuum_r2scan3c_subtractive_pilot_v1`; `scripts/mace_hybrid.py` | Four real cores completed on A5000:0.8–1.9s evaluation,1.1–1.2GiB GPU. Hybrid partition shift−5.00kcal/mol misses frozen2kcal/mol target; no accuracy/calibration claim. Versioned interface repair preserves weights/kernels/inputs. Approved memory work now tests pair/edge/node blocking on A5000 as1200381; original H200 queue entry replaced. Numerical reference agreement is gated before full-system execution. [Memory status](../diagnostics/mace_hybrid_20260916/MEMORY_STATUS.md). [Core results](../diagnostics/mace_hybrid_20260916/CORE_RESULTS.md). [Runbook](../diagnostics/mace_hybrid_20260916/RUNBOOK.md). |
