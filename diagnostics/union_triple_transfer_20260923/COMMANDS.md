...
The six finite jobs are recorded under `run_v2` and its two `searches/shard_*`
directories. Do not submit duplicates. Origins are complete; current search/cell
status is in CURRENT.md and actual receipts. These commands only inspect or
...
"$TRIPLE_PY" scripts/union_triple_transfer_compare.py \
 ...
 --output workspaces/union_triple_transfer_20260923/COMPARISON_v1.json
```
...
`run_v2/execution_v1/union_triple_transfer_run.py`; each invocation supplies the
