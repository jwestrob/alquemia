"""Allocated native/checkpointed derivatives of the unchanged masked descriptor."""
...
from mace_omol_ablation import ADAPTER as MASK, COMPONENT, receipt as mask_receipt
from mace_omol_gradients import ADAPTER, install_mask, install, receipt
...
def worker(manifest,task_id,output,memory_mode):
    ...
    try:
        ...
        r.update(charge_feature_adapter=mask_receipt(obj),output_semantics='energy_like_descriptor_not_quantum_endpoint',
        ...
        if grad:
            ...
            r.update(forces=record(fp),gradient=record(gp),force_definition='negative_Cartesian_gradient_of_masked_descriptor',
                     gradient_definition='Cartesian_gradient_of_masked_descriptor',gradient_unit='eV_equivalent_per_A',
...
def accepted(result,task):
    ...
    if (result.get('descriptor_gradient_experiment')!=task['descriptor_gradient_experiment']
        ...
        return False
    mask=result.get('charge_feature_adapter',{})
    if mask!={'id':MASK,'calls':1,'rows':s['atoms'],'output_max_abs':0.,'physical_charge_input_preserved':True,
              'spin_embedding_modified':False,'output_semantics':'energy_like_descriptor_not_quantum_endpoint'}:return False
    ...
    if grad:
        if (result.get('coordinate_derivative_method')!='analytic_autograd'
                or result.get('force_definition')!='negative_Cartesian_gradient_of_masked_descriptor'
