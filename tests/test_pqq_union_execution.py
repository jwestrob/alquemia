...
class UnionExecution(unittest.TestCase):
    def test_fixed_fresh_counts_and_profiles(self):
        ...
        for arm in e.ARMS:
            ...
            self.assertEqual(v['fresh_source_preparation_status'],'not_executed')
    ...
    def test_unexecuted_matched_comparison_keeps_strict_members(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d);paths=[]
            for arm in e.ARMS:
                out=root/(arm+'.json');e.collect(WORK/(arm+'_v4')/'plan.json',out);paths.append(out)
    ...
    def test_scientific_integration_requires_actual_execution(self):
        ...
        if not all(p.exists() for p in paths):self.skipTest('fresh matched molecular jobs have not been authorized/submitted')
        for p in paths:
            r=e.read_json(p);self.assertEqual(r['denominator'],10)
