...
def audit_preparation(path):
    ...
    if 'chain' not in selected:
        # Earlier crystal records use the equivalent normalized-selector form.
        match=re.fullmatch(r'([^:]+):([A-Z]{1,3})(-?\d+)([A-Za-z]?)/([^/]+)',
                           selected.get('normalized_selector',''))
        if match is None:raise InvalidArtifact('unsupported selected source metal selector')
        chain,resname,resnum,icode,_=match.groups()
        selected={'chain':chain,'resname':resname,'resnum':resnum,'icode':icode}
    ...
    matching=[r for r in metals if r['chain']==selected['chain'] and
              str(r['resid'])==str(selected['resnum']) and
              r['icode']==selected.get('icode',selected.get('insertion_code')) and
