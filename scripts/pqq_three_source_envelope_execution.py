...
LEGACY_PROTOCOL='Nikasha_explicit_three_La_envelope4p3_strict_native_execution_v1'
PROTOCOL='Nikasha_explicit_three_La_envelope4p3_strict_native_execution_v2'
...
def interface(protocol=PROTOCOL):
    if protocol not in (PROTOCOL,LEGACY_PROTOCOL):raise InvalidArtifact('unsupported execution interface version')
...
def checked(plan):
    ...
    if (p['protocol_id'] not in (PROTOCOL,LEGACY_PROTOCOL) or p['profile']!=PROFILE or p['resources']!=RESOURCES or
    ...
    ex=interface(p['protocol_id']);actual=ex.request_from_preparations([verify(g['preparation']) for g in req['groups']])
    if actual!=req:raise InvalidArtifact('exact prepared source request changed')
    if p['protocol_id']==LEGACY_PROTOCOL and [g['protein_id'] for g in req['groups']]!=list(PROTEINS):
        raise InvalidArtifact('historical v1 requires its fixed two PLM triples')
    if any(g['preparation_status']!='prepared' or len(g['members'])!=3 for g in req['groups']):
        raise InvalidArtifact('each explicitly declared group requires three complete prepared sources')
    count=len(req['cases'])
    if count!=3*len(req['groups']) or p['case_ids']!=[c['case_id'] for c in req['cases']] or p['declared_calls']!=ex.declared_calls(count):
        raise InvalidArtifact('finite source/group/call scope differs')
...
def engine(plan):
    protocol=read_json(plan)['protocol_id'];ex=interface(protocol);own=union.private('pqq_union_execution')
...
def prepare(preparations,strict_qualification,rank_qualification,maxiter_qualification,agreement,output):
    ...
    p={'protocol_id':PROTOCOL,'profile':PROFILE,'request':record(out/'REQUEST.json'),'release':req['release'],'reference':req['reference'],
       ...
       'declared_calls':ex.declared_calls(len(req['cases'])),'optimizer_software':{'version':scipy.__version__,
            'wrapper':record(slsqp_wrapper.__file__),'kernel':record(slsqp_kernel.__file__)},
       'source_domain_status':{c['case_id']:'experimental_protocol_compatible_biological_domain_unvalidated' for c in req['cases']},
...
def dry_run(plan):
    p,req,_,_=checked(plan);result=interface(p['protocol_id']).dry_run(plan);count=len(req['cases'])
    if result['origin_MACE_tasks']!=2*count or result['origin_GFN2_tasks']!=4*count:
        raise InvalidArtifact('all explicitly declared paired origins must be complete')
...
def execute(plan):
    ...
    return interface(p['protocol_id']).execute(plan)


def collect(plan,output):
    protocol=read_json(plan)['protocol_id'];ex=interface(protocol);raw=Path(output).with_name(Path(output).stem+'__engine.json')
    ...
    d.update(engine_collection=record(raw),candidate_status='experimental_not_promoted',
             ...
             scope=('exact_two_consumed_unlabeled_PLM_triples_usability_not_accuracy' if protocol==LEGACY_PROTOCOL
                    else 'explicit_complete_three_source_groups_experimental_prediction_not_accuracy'),
...
def report(result,output):
    d=read_json(result);p,_,_,_=checked(verify(d['plan']))
    ex=interface(p['protocol_id']);r=ex.report(result,output)
    ...
    with Path(output).open('a') as f:
        f.write('\n\nStrict native scalar profile: '+PROFILE+'.\n'
                'Explicit source predictions are experimental, not independent biological accuracy validation.\n'
