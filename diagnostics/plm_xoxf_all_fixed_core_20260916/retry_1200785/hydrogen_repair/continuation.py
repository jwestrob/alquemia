...
OLD=BASE/'prepared_batch';FIRST=BASE/'hydrogen_repair';OUT=FIRST/'continuation'
...
def recover():
    ...
    restart=FIRST/'recovered_full_precision_positions_nm.npy';previous=read(FIRST/'recovery_validation.json')
    if previous['status']!='FAIL' or previous['iterations_reported']!=169:raise ValueError('Unexpected first recovery outcome')
    b.verify(previous['full_precision_positions']);b.verify(previous['captured_original_System'])
    saved=np.load(restart)
    if saved.shape!=(len(atoms),3):raise ValueError('Restart positions differ')
    ctx.setPositions(saved*unit.nanometer);before,bm=h.state(ctx,mask);reporter=h.Reporter();start=time.monotonic()
    mm.LocalEnergyMinimizer.minimize(ctx,0.5,831,reporter)
    ...
    result={'status':'PASS' if identical_heavy and am['hydrogen_force_rms_kJ_mol_nm']<=1.0 else 'FAIL',
      ...
      'tolerance':0.5,'unchanged_H_only_acceptance_tolerance':1.0,'maximum_iterations':831,'iterations_reported':len(reporter.entries),
      'previous_attempt':record(FIRST/'recovery_validation.json'),'restart_positions':record(restart),'total_recovery_iterations':169+len(reporter.entries),
      ...
      'preparation_deviation':'Same captured objective; continue retained full-precision H coordinates after169 iterations; internal tolerance1.0→0.5 while H-only acceptance remains1.0; at most831 further steps; no fresh protonation',
